...
"""Race track goals outside q1"""
GOAL_LIST = [[8.65,0.16],[19.5,-0.21],[14.1,4.07],[-0.91,5.34],[-7.81,0.73],[2.69,0.92]]

def print_tree(t): # print behaviour-tree
    print("\033[2J\033[H")
    pt.display.print_ascii_tree(t, show_status=True)

class BehaviourTree(pt.trees.BehaviourTree):
    """
    Behaviour-tree defining the behaviour of the svea car.
    Make sure car follows waypoints, detects obstacles, plans and drives.
    """
...
def main():
    ...
    # Give car initial position in rviz when running the code
    rospy.loginfo('Waiting for initial position...')
    rospy.wait_for_message('/initialpose', PoseWithCovarianceStamped)

    # Publisher for waypoints from global planner (purple diamonds in rviz)
    ...
    # # Use RRT for planning between target points in GOAL_LIST
    ...
    # Hard-coded paths
    path = [[2.62257741e-02, 5.55977173e-06],[1.55,-0.09],[3.41, 2],[6.6, 1.91],[8.65, 0.16],[12.9, -0.96],[16.74991176, -1.26631695],[19.5 , -0.21],[19.40077279,  3.11716495],[16.93911541,  3.97146113],[14.1 ,  4.07], [10.9, 4.48],[7.14921592, 4.52974599],[4.92176179, 6.79254158],[2.96389709, 6.9894187 ],[0.73324065, 5.37245639],[-0.91,  5.34],[-4.64,5.45],[-9.18432957,  5.57541465],[-13,6.21],[-14.1,4.98],[-14.2,2.18],[-13.4,0.832],[-10.48376699,   0.75753486],[-8.77,0.552],[-7.59,1.52],[-7.55,3.38],[-5.33,3.28],[-5.54,0.48],[-1.89603235,  0.05966632],[1.0222541 , 0.17457613],[2.69, 0.92]] # Counter clockwise path
    # path = [[-1.89603235, 0.05966632], [-5.54, 0.48], [-5.33, 3.28], [-7.55, 3.38], [-7.59, 1.52], [-8.77, 0.552], [-10.48376699, 0.75753486], [-13.4, 0.832], [-14.2, 2.18], [-14.1, 4.98], [-13, 6.21], [-9.18432957, 5.57541465],[-4.64, 5.45], [-0.91, 5.34], [0.73324065, 5.37245639], [2.96389709, 6.9894187], [4.92176179, 6.79254158], [7.14921592, 4.52974599], [10.9, 4.48], [14.1, 4.07], [16.93911541, 3.97146113], [19.40077279, 3.11716495], [19.5, -0.21], [16.74991176, -1.26631695], [12.9, -0.96], [8.65, 0.16], [6.6, 1.91], [3.41, 2], [1.55, -0.09], [0.0262257741, 5.55977173e-06], [-1.89603235, 0.05966632]] # Clockwise path
