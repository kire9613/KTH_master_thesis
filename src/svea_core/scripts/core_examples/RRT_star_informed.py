...
class InformedRRTStar:
    ...
    def __init__(self, start, goal,
                 ...
                 expandDis=0.9, goalSampleRate=10, maxIter=200): # changed from 0.5 to 0.9
    ...
    def sample_free_space(self):
        if random.randint(0, 10) > self.goal_sample_rate: # changed from 100
...
def get_obs(obs_matrix):
    ...
    for n in range(200, 878): #
        for m in range(0, 170): #
            ...
            if value > 10:
                obslist.append([n*0.05, m*0.05, 0.05])
    return obslist
...
def main():
    ...
    obstacleList = get_obs(map_matrix)


    # Set Initial parameters
    start = [15, 1.54 , np.deg2rad(90)] # 0.0
    goal = [37, 1.44 , np.deg2rad(0.0)]
    ...
    rrt = InformedRRTStar(start, goal,
                          randArea=[-2, width*resolution], obstacleList=obstacleList)
