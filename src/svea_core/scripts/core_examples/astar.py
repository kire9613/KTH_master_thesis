...
#		oy.append(60.0 - i)
	...
	obstacle_map = pickled_problem.reshape(map_height, map_width)
	...
	for i in range(map_height):
		for j in range(map_width):
			if obstacle_map[i,j] == bounding_box:
				if i < minx:
					minx = i
				if j < miny:
					miny = j
	...
	for i in range(minx-1,maxx + 1):
		for j in range(miny-1,maxy + 1):
			x=i
			y=j
			if obstacle_map[i,j] == occupied_space: 
				ox.append(x)
				oy.append(y)
			if obstacle_map[i,j] == start_spot:
				sx = x
				sy = y
			if obstacle_map[i,j] == goal_spot:
				gx = x
				gy = y
