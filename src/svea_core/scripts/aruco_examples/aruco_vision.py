...
rospy.init_node('aruco_vision')
aruco_detect = ArucoInterface(vehicle_frame='base_link', camera_frame='webcam').start()

vehicle_state = State() # Defaults to origin
posepub = rospy.Publisher('/observed_pose', PoseStamped, queue_size=10)
markerpub = rospy.Publisher('/marker_list', Bool, queue_size=10)
...
def main():
    ...
    """
    add_marker_to_dictionary(marker_dict, 1)
...
def publish_detected_markers():
    ...
    markerpub.publish(markers_visible)
