    ...
    def get_inflated_map(self,msg):
        ...
        for i in range(0,len(self.current_path.x_coordinates)):    
            ...
            if check and not self.sent and not self.remote_overwrite:
                ...
                #TODO: implement A* interaprot and reinitiator if estimation fails
                """signal.alarm(5)
                try:
                    new_path = path_requester.estimate_path([self.current_x, self.current_y],[target_x,target_y], map.data, width, height)
                ...
                if self.obs_N > 0:
                    ...
                    #for j in range(0, len(self.traj_x) -1 ):
                    #    x = int((self.traj_x[j] - origin_x)/resolution)
                    #    y = int((self.traj_y[j] - origin_y)/resolution)
                    #    inflated_map[int(y),int(x)] = 250
                    plt.imshow(inflated_map)
                    plt.colorbar()
                    plt.show()"""
...
def handler(signum, frame):
        print("A* can can not calculate the path. Trying different target point...")
