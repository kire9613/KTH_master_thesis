...
class RrtStar:
    def __init__(self, x_start, x_goal, step_len,
                 goal_sample_rate, search_radius, iter_max, obstacles, grid):
        ...
        self.utils = utils.Utils(obstacles, grid)
    ...
    def generate_random_node(self, goal_sample_rate):
        ...
        if np.random.random() > goal_sample_rate:
            ...
            rand_y = np.random.uniform(1.46215*rand_x + 6.295, 1.46215*rand_x - 8.963)
...
def compute_path_star(x0, y0, xt, yt, obstacles, grid):
    x_start = (x0, y0)  # Starting node
    x_goal = (xt, yt)  # Goal node

    iter_max = 5000
    rrt_star = RrtStar(x_start, x_goal, 0.5, 0.10, 5, iter_max, obstacles, grid)
